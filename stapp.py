...
# Define transformations (match the ones used during training)
...
# Load the model
model = KAN([32 * 32 * 3, 64, 3])  # Ensure the model is initialized correctly
...
def predict_image(image):
    ...
    # Reshape the image if necessary (e.g., flattening for fully connected layers)
    if isinstance(model.layers[0], KANLinear):
        image = image.view(-1, 32 * 32 * 3)  # Flatten to (1, 32*32*3)
...
# Streamlit app
st.title("Fridha Megantara Putra")
st.title("COVID-19 Pneumonia Classification")
st.write("Upload an X-ray image to classify or select a sample image")

# Option to select sample image or upload own
option = st.selectbox(
    "Choose an option:",
    ["Select a sample image", "Upload your own image"]
)

if option == "Select a sample image":
    sample = st.selectbox(
        "Choose a sample image:",
        ["covid.jpeg", "normal.jpeg", "pneumonia.jpeg"]
    )
    if sample:
